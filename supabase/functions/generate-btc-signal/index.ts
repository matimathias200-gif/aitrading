...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    console.log('[generate-btc-signal] Starting with enhanced prompt...');
    ...
    const dominance = market.dominance || 45;
    ...
    const prompt = `Tu es un analyste crypto professionnel spécialisé en BTCUSDT.

Analyse le marché du Bitcoin sur plusieurs horizons de temps (1h, 4h, 1j) en tenant compte des aspects techniques, fondamentaux, on-chain et du sentiment global.
...
Données de marché :
{
  ...
  "btc_funding_rate": ${fundingRate.toFixed(4)}
}
...

{
  ...
  "reason": {
    ...
    if (signal.signal_type === 'BUY' || signal.signal_type === 'SELL') {
      ...
      saved = savedData;
      console.log(`[generate-btc-signal] Signal saved: ${signal.signal_type} @ ${signal.entry_price} (confidence: ${signal.confidence}%)`);
    } else {
